...
async def create_SpiderMan(request: Request,
    ...
    try:
        new_spider = spiderManCreate(name=name, alias=alias, skills=skills, alive=is_alive, img=img_url,
...
async def edit_spiderMan_form(request: Request, spiderMan_id: int, session: SessionDep):
    ...
    if not spiderMan_db:
        raise HTTPException(status_code=404, detail="SpiderMan not found")
    return Templates.TemplateResponse("edit_spiderMan.html", {"request": request, "SpiderMan": spiderMan_db,
                                                              "universe_id": spiderMan_db.universe_id})
